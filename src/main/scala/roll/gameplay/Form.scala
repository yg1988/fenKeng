...
//    dom.console.log("processElement", elem)
    elem match{
      ...
      case Xml.Circle(x, y, r, misc) =>
        ...
        val (friction, density, elasticity)  = if (misc.fill != "") splitFill(misc.fill) else splitFill("#FF8877")
  ...
  def setRandomColor={
    "#Fe"+ ("000000" + (Math.random()*0xFFFF).toInt.toString).reverse.substring(0,4)
  }
}
